(ns advent-of-code-2024.10.journal
...
(comment
  ;; going to continue to assume this grid is square bc it has been true for all past puzzles
  ...
  (defn trailheads [topo-map]
    (let [size (count topo-map)]
      (for [i (range size)
        ...
        [i j])))
  (def test-trailheads (trailheads test-input))
  ...
  ;; returns only neighbors that are in bounds
  ...
  (let [topo-map test-input, trailhead [0 6]]
    (let [pos trailhead, to-visit [], visited #{}, score 0]
  ...
  ;; => 36
  ;; okay got it for the test input
  ;; actual input:
